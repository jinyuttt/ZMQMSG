...
public  void start()
{
	...
	if(msg.startsWith("sub")||msg.startsWith("pub"))
	{
		...
		if(msg.startsWith("sub"))
		{
			...
			//
			String ip=Tool.getLocalHostLANAddress();
			if(addr.contains("*:"))
			{
				addr=addr.replace("*:", ip+":");
			}
		}
		else
		{
	...
	else
	{
		...
		{
	         System.out.println("服务接收数据错误："+ex.getMessage());
			...
			if(reqSuecess)
			{
				       Thread startNode=new Thread(new Runnable() {
						public void run() {
							...
							nodeServer.cascadedNodeIP=AppConfig.cascadedNodeIP;
							nodeServer.cascadedNodePort=AppConfig.cascadedNodePort;
					    	System.out.println("启动本节点级联订阅");
					    	String logmsg="更新级联全数据订阅";
							LogFactory.getInstance().addInfo(logmsg);
							nodeServer.zmq_pub_sub_kz();
